  ...
  if (slots.full) {
    return (
      <div className="rounded-xl overflow-hidden border border-primary-200">
        <div className={`px-1.5 py-1 text-center ${isToday ? 'bg-primary-500' : 'bg-gray-50 border-b border-gray-100'}`}>
          <span className={`text-xs font-bold ${isToday ? 'text-white' : 'text-gray-700'}`}>{dateStr}</span>
        </div>
        <div className="bg-primary-50 px-1.5 py-2 text-center">
  ...
  return (
    <div className="rounded-xl overflow-hidden border border-gray-100">
      ...
      <div className="divide-y divide-gray-100">
        {/* Day shift */}
        {slots.day ? (
          <div className="bg-amber-50 px-1.5 py-1.5 text-center">
            <p className="text-[9px] font-bold text-amber-700">13–19</p>
            <p className="text-[8px] text-amber-600 truncate">{slots.day.guest.split(' ')[0]}</p>
          </div>
        ) : (
          <button onClick={() => !isPast && onBook('day')}
            disabled={isPast}
            className="w-full bg-white px-1.5 py-1.5 text-center hover:bg-amber-50 transition disabled:opacity-30">
            <p className="text-[9px] text-gray-400">13–19</p>
            <p className="text-[8px] text-emerald-500 font-medium">св.</p>
          </button>
        )}
        {/* Night shift */}
        {slots.night ? (
          <div className="bg-indigo-50 px-1.5 py-1.5 text-center">
            <p className="text-[9px] font-bold text-indigo-700">20–11</p>
            <p className="text-[8px] text-indigo-600 truncate">{slots.night.guest.split(' ')[0]}</p>
          </div>
        ) : (
          <button onClick={() => !isPast && onBook('night')}
            disabled={isPast}
            className="w-full bg-white px-1.5 py-1.5 text-center hover:bg-indigo-50 transition disabled:opacity-30">
            <p className="text-[9px] text-gray-400">20–11</p>
            <p className="text-[8px] text-emerald-500 font-medium">св.</p>
          </button>
        )}
        ...
        {!slots.day && !slots.night && !isPast && (
          <button onClick={() => onBook('full')}
            className="w-full bg-gray-50 px-1 py-1 text-center hover:bg-primary-50 transition">
            <p className="text-[8px] text-gray-400 font-medium">сутки</p>
          </button>
        )}
      </div>
    </div>
...
export default function CottagePage() {
  ...
  return (
    <div className="px-4 py-4 space-y-3">
      ...
      {/* Legend */}
      <div className="bg-white rounded-2xl shadow-card px-4 py-3">
        <p className="text-xs font-semibold text-gray-500 uppercase mb-2">Легенда</p>
