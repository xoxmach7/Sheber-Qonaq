...
import { Plus, X, LogOut, User, Search, AlertTriangle, UserPlus, ChevronLeft, CreditCard, CalendarClock, Banknote, Smartphone, Building2 } from 'lucide-react'
import StatusBadge from '../../components/StatusBadge'
import type { StayCreate, Stay, PaymentCreate, PaymentMethod, RateType, GuestCreate } from '../../types'
...
function ExtendForm({ stay, onClose }: { stay: Stay; onClose: () => void }) {
  ...
  const [newDate, setNewDate] = useState(() => {
    // По умолчанию +30 дней от текущей планируемой даты
    const d = new Date(stay.expected_check_out_date)
...
function CheckInForm({ onClose }: { onClose: () => void }) {
  ...
  // Проверка чёрного списка при выборе гостя
  ...
  // Быстрое создание гостя прямо в форме заселения
              ...
              <div className="relative">
                ...
                {showDropdown && guestSearch.length >= 2 && guestResults?.results?.length === 0 && (
                  <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden">
                    ...
                    <button
                      onMouseDown={() => {
                        // Предзаполнить телефон если ввели телефон
            ...
            return (
              <div
                ...
                className={`bg-white rounded-2xl shadow-sm border overflow-hidden ${
                  isDebt ? 'border-red-200' : 'border-gray-100'
                }`}
              >
                {/* Guest + unit header */}
